...
const PurchaseModal: React.FC<ModalProps> = ({ isOpen, product, onClose }) => {
  ...
  const handlePurchase = () => {
    if (product && selectedCountry && selectedDuration) {
      const countryPrices = product.prices[selectedCountry];
      const price = countryPrices[selectedDuration as keyof typeof countryPrices];
      const currency = countryPrices.currency;
...
const HomePage = () => {
  ...
  const products: Product[] = [
    ...
    },
    {
      ...
      prices: {
        PK: { "1 Month": 1000, "3 Months": 2700, "6 Months": 5000, "12 Months": 10000, currency: "PKR" },
        UK: { "1 Month": 6, "3 Months": 18, "6 Months": 30, "12 Months": 55, currency: "£" },
  ...
  <input
    ...
    onKeyDown={(e) => {
      if (e.key === "Enter") {
        ...
        e.currentTarget.blur(); // Deselect the search bar
      }
    }}
  />
